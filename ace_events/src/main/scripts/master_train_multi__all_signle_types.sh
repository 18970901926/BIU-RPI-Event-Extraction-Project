...
TRAIN_DOCS=src/main/resources/doclists/new_filelist_ACE_training.txt
DEV_DOCS=src/main/resources/doclists/new_filelist_ACE_dev.txt

ARGS="beamSize=4 maxIterNum=20 skipNonEventSent=true avgArguments=true skipNonArgument=true useGlobalFeature=false addNeverSeenFeatures=true crossSent=false crossSentReranking=false order=0 evaluatorType=1 learnBigrams=true logLevel=1 useArguments=false"
...
TYPE_NUM=01
SINGLE_TYPE=Attack
RUN=$DATE_TITLE..$TYPE_NUM--master-Train--only-$SINGLE_TYPE--TESRV2
RUN_FULL=/home/ir/bronsho/runs/Research/$RUN
mkdir -p $RUN_FULL
LOGNAME=master-train-$SINGLE_TYPE.log
DATE=$(date +"%F %T"); echo "$DATE Running $TYPE_NUM. $SINGLE_TYPE"
MODEL=$RUN_FULL/model_NTP
mvn exec:java -Dexec.mainClass=edu.cuny.qc.perceptron.core.Pipeline -Dexec.args="$CORPUS $TRAIN_DOCS $MODEL $DEV_DOCS $SINGLE_TYPE $ARGS" 2>&1 | tee $LOGNAME > $RUN_FULL/$LOGNAME
