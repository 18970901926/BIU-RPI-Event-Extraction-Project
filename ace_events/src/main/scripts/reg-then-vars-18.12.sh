...
echo date +"[%D %T] Running preprocess of train..." >> $PROGRESS_PATH
echo java -server -Xmx20g ac.biu.nlp.nlp.engineml.rteflow.systems.rtepairs.RTEPairsPreProcessor $RUN/biutee_train.xml
echo date +"[%D %T] Done preprocess of train." >> $PROGRESS_PATH

echo date +"[%D %T] Running preprocess of test..." >> $PROGRESS_PATH
echo java -server -Xmx20g ac.biu.nlp.nlp.engineml.rteflow.systems.rtepairs.RTEPairsPreProcessor $RUN/biutee_test.xml
echo date +"[%D %T] Done preprocess of test." >> $PROGRESS_PATH
...
date +"[%D %T] Running vars on RTE3_test___vars_mini.xml..." >> $PROGRESS_PATH
java -server -Xmx20g ac.biu.nlp.ie_over_entailment.VarExperiment $RUN/excitement_biu.xml "biutee" $RUN/RTE3_test___vars_mini.xml
date +"[%D %T] Done vars." >> $PROGRESS_PATH
